...
while True:
    try:
        ...
        apiurl = 'https://api.bscscan.com/api?module=account&action=txlist&address=' + \
            token+'&startblock='+pb+'&endblock='+cb+'&sort=asc&apikey='+api
        l = (requests.get(apiurl)).text
        t = l.count('hash')
        if burn == '':
            pass
        else:
            burnurl = 'https://api.bscscan.com/api?module=account&action=txlist&address=' + \
                burn+'&startblock='+pb+'&endblock='+cb+'&sort=asc&apikey='+api
        ...
        while t < 16:
            ...
            print(
                f"Checking block {pb} to {cb} ({str(int(cb)-int(pb))}) - Txs to check: {t}", end="\r")
            '''
        ...
        if burn == '':
            ...
            for c in range(tb):
                # the cap is 5 requests per second. This makes 3 requests per second at max.
                sleep(0.33)
                pos = burnl.find('hash')
                burnl = burnl[pos+4:]
                pos = burnl.find('0')
            ...
            try:
                ...
                if dolla >= 100:
                    ...
                    if hw.count('Binance: Hot Wallet') == 0:
                        ...
                        pos = hw.find(
                            "From</b> </span><span class='hash-tag text-truncate  mr-1'><a href='/token/")
                        hw = hw[pos:]
